...
def send_heartbeat():
    while True:
        try:
            ...
            requests.post(url, json=heartbeat_data, timeout=3)
    ...
    try:
        ...
        if 'matrixData' not in task_data:
            if 'matrixDimension' in task_data:
                ...
                sub_size = end_row - start_row
                task_data['matrixData'] = create_random_matrix(sub_size, symmetric=True).tolist()
...
def cancel_task(subtask_id):
    if task_executor.cancel_task(subtask_id):
