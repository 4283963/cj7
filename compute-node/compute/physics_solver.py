import numpy as np
from .eigenvalue import compute_eigenvalues
...
def schrodinger_solver(potential, n_points=1000, n_eigenvalues=10):
    ...
    eigenvalues, eigenvectors = compute_eigenvalues(H, k=n_eigenvalues, tol=1e-10)
...
def structural_dynamics_solver(n_elements=100, n_modes=5):
    K, M = finite_element_stiffness(n_elements)
...
def apply_physics_formula(matrix, formula_type):
    if formula_type == 'schrodinger':
        eigenvalues, eigenvectors, _ = schrodinger_solver(lambda x: 0.5 * x**2, n_points=matrix.shape[0])
        return eigenvalues, eigenvectors
    elif formula_type == 'structural':
        n = matrix.shape[0]
        K = matrix + np.eye(n) * 10
        M = np.eye(n)
        eigenvalues, eigenvectors = compute_eigenvalues(np.linalg.inv(M) @ K, k=10)
        return eigenvalues, eigenvectors
    elif formula_type == 'helmholtz':
        k = 2 * np.pi
        eigenvalues, eigenvectors = compute_eigenvalues(matrix, k=10)
        return eigenvalues, eigenvectors
    else:
        return compute_eigenvalues(matrix, k=10)
